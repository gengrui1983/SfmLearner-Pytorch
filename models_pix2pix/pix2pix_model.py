...
class Pix2PixModel(BaseModel):
            ...
            for i in range(self.opt.num_D):
                for j in range(len(self.pred_fake[i]) - 1):
                    self.loss_G_GAN_Feat += D_weights * feat_weights * \
                                            self.criterionL1(self.pred_fake[i][j],
                                                             self.pred_real[i][j].detach()) * self.opt.lambda_feat
    ...
    def get_current_visuals(self):
        ...
        print("shapes:\nrealA:{} fakeB:{} realB:{} ".format(
            real_A.shape, fake_B.shape, real_B.shape
        ))

        return OrderedDict([('real_A', real_A[:, :, :3]), ('fake_B', fake_B), ('real_B', real_B)])
